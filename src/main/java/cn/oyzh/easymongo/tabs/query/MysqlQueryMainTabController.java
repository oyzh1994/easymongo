...
public class MysqlQueryMainTabController extends RichTabController {
     ...
     * @param dbItem db库树节点
     */
    public void init(MysqlQueryMainTab tab, MongoQuery query, MongoDatabaseTreeItem dbItem) {
        this.tab = tab;
        this.query = query;
        this.dbItem = dbItem;
        this.showNode(0);
    ...
    private void save() {
        try {
            ...
            // 新增查询
            if (this.query.isNew()) {
                result = MongoQueryStore.INSTANCE.insert(this.query);
                if (result) {
                    this.dbItem.getQueryTypeChild().addQuery(this.query);
                }
            } else {// 修改查询
                result = MongoQueryStore.INSTANCE.update(this.query);
            }
            if (!result) {
