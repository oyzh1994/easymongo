...
public class ShellMongoDataTransportController extends StageController {
     ...
     */
    @FXML
    private MongoConnectComboBox sourceInfo;

    /**
     * 目标信息
     */
    @FXML
    private MongoConnectComboBox targetInfo;
    ...
    protected void bindListeners() {
        super.bindListeners();
        this.sourceInfo.selectedItemChanged((obs,oldVal,newValue) -> {
            StageManager.showMask(() -> this.doConnect(1, newValue));
        });
        this.targetInfo.selectedItemChanged((obs,oldVal,newValue) -> {
     ...
     * @param connect 连接
     */
    private void doConnect(int type, MongoConnect connect) {
        try {
            if (type == 1) {
                if (connect != null) {
    ...
    public void onWindowShown(WindowEvent event) {
        ...
        MongoConnect connect = this.getProp("connect");
        if (connect != null) {
            this.sourceInfo.select(connect);
        }
    }
    ...
    private void showStep2() {
        ...
        MongoConnect targetInfo = this.targetInfo.getSelectedItem();
